...
export const useCardsStore = create()(devtools((set, get) => ({
    initialized: false,
    loading: false,
    cards: [],
    categories: [],
    ...
    initialize: async () => {
        ...
        const logsByCard = logs.reduce((acc, log) => {
            if (!acc[log.cardId])
                acc[log.cardId] = [];
    ...
    createCategory: async (name) => {
        const category = { id: safeId('cat'), name };
        await db.categories.put(category);
        set((state) => ({ categories: [...state.categories, category] }));
        return category;
    },
    createTag: async (name) => {
        const tag = { id: safeId('tag'), name };
    ...
    logReview: async (cardId, quality, reflection) => {
        ...
        set((current) => ({
            cards: current.cards.map((item) => item.id === cardId
                ? {
                    ...item,
                    mastery,
                    nextReviewAt: updatedSchedule.nextReviewAt,
    ...
    addReflection: async (cardId, content) => {
        ...
        set((state) => ({
            cards: state.cards.map((item) => cardId === item.id ? { ...item, reflections: [...item.reflections, note] } : item)
        }));
    },
    addActionPrompt: async (cardId, prompt) => {
        const action = { id: safeId('act'), prompt };
        const card = get().cards.find((item) => item.id === cardId);
        if (!card)
            return;
        await db.cards.update(cardId, {
        ...
        set((state) => ({
            cards: state.cards.map((item) => item.id === cardId ? { ...item, actions: [...item.actions, action] } : item)
        }));
    }
})));
export function selectDueCards(cards, schedules) {
...
export function selectUpcoming(cards, schedules) {
