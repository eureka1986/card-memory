...
const EASE_MIN = 1.3;
export function evaluateMastery(quality, current) {
    if (quality >= 4) {
        if (current === 'unseen')
            return 'learning';
        if (current === 'learning')
...
export function nextSchedule(quality, ctx) {
    const now = new Date();
    if (!ctx.schedule) {
...
export function needsReview(schedule, date = new Date()) {
    return isBefore(new Date(schedule.nextReviewAt), addDays(date, 1));
}
export function overdueDays(schedule, date = new Date()) {
